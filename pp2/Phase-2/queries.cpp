...
json handleKShortestPathsExact(Graph& g, const json& query) {
    ...
    vector<Path> paths = g.kShortestPathsExact(source, target, k, mode);

    json arr = json::array();
    for (auto& p : paths) {
        arr.push_back({{"path", p.nodes}, {"length", p.length}});
    }
    return arr;
}
...
json handleKShortestPathsHeuristic(Graph& g, const json& query) {
    ...
    int target = query["target"];
    int k = query["k"];
    int overlap_threshold = query.value("overlap_threshold", 60);
    string mode = query.value("mode", "distance");

    vector<Path> paths = g.kShortestPathsHeuristic(source, target, k, overlap_threshold, mode);
...
json handleApproxShortestPaths(Graph& g, const json& query) {
    vector<pair<int,int>> queries_vec;
    for (auto& q : query["queries"]) {
        queries_vec.push_back({q["source"], q["target"]});
    }

    int time_budget = query.value("time_budget_ms", 10);
    double acceptable_error = query.value("acceptable_error_pct", 5.0);
    string mode = query.value("mode", "distance");

    vector<double> dists = g.approxShortestPaths(queries_vec, time_budget, acceptable_error, mode);

    json arr = json::array();
    for (size_t i = 0; i < queries_vec.size(); i++) {
        arr.push_back({
            {"source", queries_vec[i].first},
            {"target", queries_vec[i].second},
            {"approx_shortest_distance", dists[i]}
        });
    }
...
json process_query(Graph& g, const json& query) {
    ...
    // 5️⃣ PHASE-2: K SHORTEST PATHS (EXACT)
    else if (type == "k_shortest_paths") {
        result["paths"] = handleKShortestPathsExact(g, query);
        return result;
    }

    // 6️⃣ PHASE-2: K SHORTEST PATHS (HEURISTIC)
    else if (type == "k_shortest_paths_heuristic") {
        result["paths"] = handleKShortestPathsHeuristic(g, query);
        return result;
    }

    // 7️⃣ PHASE-2: APPROXIMATE SHORTEST PATHS
    else if (type == "approx_shortest_path") {
        result["distances"] = handleApproxShortestPaths(g, query);
        return result;
    }

...
/*
    ...
    Added optional 'mode' parameter: "distance" or "time"
*/
vector<pair<int, double>> KNN_ShortestPath(const Graph& g, int source, int k, const string& mode = "distance") {
    ...
    using P = pair<double, int>; // (distance/time, node)
    ...
    while (!pq.empty()) {
        ...
        for (const auto& edge : g.adj.at(u)) {
            ...
            int v = edge.v;
            double w = (mode == "time") ? edge.average_time : edge.length;  // Use mode
    ...
    // Sort by distance/time and trim to k nearest
